...
게이트는 디스크립터 권한 레벨 DPL을 가지며, 
이는 프로세서가 DPL 또는 그보다 낮은 번호의 링에 있을 때 
의도적으로 호출될 수 있다는 것을 의미합니다.
실제로, DPL==3은 사용자 모드에서 게이트를 호출할 수 있게 하고, 
DPL==0은 이러한 호출을 방지합니다.
사용자 모드에서 발생하는 오류와 예외는 여전히 DPL==0인 게이트를 호출하게 됩니다.

TYPE은 14(인터럽트 게이트) 또는 15(트랩 게이트)이어야 합니다.
차이점은 인터럽트 게이트에 진입하면 인터럽트가 비활성화되지만,
 트랩 게이트에 진입하면 인터럽트가 비활성화되지 않는다는 것입니다.
자세한 내용은 [IA32-v3a] 섹션 5.12.1.2 "Flag Usage By Exception- 
or Interrupt-Handler Procedure"을 참조하십시오. */
struct gate {
...
/* 인터럽트 디스크립터 테이블(IDT)입니다. 형식은 CPU에 의해 고정됩니다.
자세한 내용은 [IA32-v3a] 섹션 5.10 "Interrupt Descriptor Table (IDT)",
5.11 "IDT Descriptors", 
5.12.1.2 "Flag Usage By Exception- or Interrupt-Handler Procedure"을 참조하십시오. */
...
/* External interrupts are those generated by devices outside the
   ...
   interrupt returns. */
/* 외부 인터럽트는 타이머와 같은 CPU 외부의 장치에 의해 생성됩니다.
외부 인터럽트는 인터럽트가 비활성화된 상태에서 실행되므로 중첩되지 않으며,
 선점되지도 않습니다.
외부 인터럽트의 핸들러는 슬립하지 않지만, 
인터럽트가 반환되기 바로 직전에 intr_yield_on_return()을 호출하여
새로운 프로세스가 스케줄되도록 요청할 수 있습니다. */
static bool in_external_intr;   /* Are we processing an external interrupt? */ /* 현재 외부 인터럽트를 처리 중인가요? */
static bool yield_on_return;    /* Should we yield on interrupt return? */

/* Programmable Interrupt Controller helpers. */
/* 프로그래밍 가능한 인터럽트 컨트롤러 도우미 함수들 */
...
intr_get_level (void) {
	...
	스택에서 flags로 값을 팝합니다. 
	[IA32-v2b]의 "PUSHF"와 "POP", 그리고 
	[IA32-v3a]의 5.8.1 "Masking Maskable Hardware Interrupts"를 참조하세요. */
...
/* 인터럽트 VEC_NO를 HANDLER로 등록하고, 디스크립터 권한 레벨 DPL로 설정합니다.
디버깅을 위해 인터럽트의 이름을 NAME으로 지정합니다.
인터럽트 핸들러는 LEVEL로 인터럽트 상태를 설정하여 호출됩니다. */
static void
...
디버깅 목적으로 NAME이라는 이름을 갖는 핸들러가 실행됩니다.
...
인터럽트에서 반환하기 직전에 새로운 프로세스에 양보하도록 지시합니다. 
다른 시간에는 호출되지 않을 수 있습니다. */
void
