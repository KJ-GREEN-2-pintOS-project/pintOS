...
__do_fork(void *aux) {
	...
	struct intr_frame *parent_if = &parent->ptf; // ptf 사용
	bool succ = true;

	/* 1. Read the cpu context to local stack. */
	memcpy(&if_, parent_if, sizeof(struct intr_frame));
	if_.R.rax = 0; // rax 값 초기화
	...
	sema_up(&parent->sema_fork); // sema_up 추가
...
void process_exit(void) {
	...
	sema_up(&curr->sema_wait); // sema_up 추가
	sema_down(&curr->sema_exit); // sema_down 추가

	palloc_free_page(table); // palloc 함수 호출 추가
	process_cleanup();
}
